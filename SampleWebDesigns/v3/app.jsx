// Multi-step waitlist onboarding
const GOALS = [
  { e: '🧹', l: 'Declutter' },
  { e: '💰', l: 'Make cash' },
  { e: '📦', l: 'Moving soon' },
      ...
      <div className="onb" onClick={e => e.stopPropagation()}>
        ...
        <div className="onb-left">
          <div>
            ...
            <h3 className="onb-title" dangerouslySetInnerHTML={{ __html: s.leftT }}/>
            <div className="onb-sub">{s.leftS}</div>
          </div>
          <div style={{ opacity: 0.6, position: 'relative', zIndex: 2 }}><MascotV2 size={60} color="var(--before-accent)" /></div>
        </div>
...
const App = () => {
  const [tweaks, setTweaks] = React.useState(() => window.TWEAK_DEFAULTS_V3 || { palette: 'clay' });
  ...
  const openOnb = () => setShowOnb(true);

  return (
    <>
      <nav className="nav">
        <div className="nav-logo">bounce<span className="dot">.</span></div>
        <div className="nav-links">
          <a href="#how">How it works</a>
          <a href="#faq">FAQ</a>
          <a href="#" onClick={e => { e.preventDefault(); openOnb(); }}>Pricing</a>
        </div>
        <button className="nav-cta" onClick={openOnb}>Join waitlist</button>
      </nav>

      <Hero onJoin={openOnb} />
      <Numbers onJoin={openOnb} />
      <HowItWorks onJoin={openOnb} />
      <Estimator onJoin={openOnb} />
      <Carousel />
      <Testimonials onJoin={openOnb} />
      <FAQ />
      <Finale onJoin={openOnb} />

      <footer className="footer">
        <div className="l">bounce<span style={{ color: 'var(--before-accent)' }}>.</span></div>
        <div className="links">
          <a href="#">Privacy</a>
          <a href="#">Terms</a>
          <a href="#">Contact</a>
      ...
      {tweaksOn && (
        <>
          <div className="tweaks-panel">
            <h3>Tweaks</h3>
            <div className="tw-sub">Horizontal split · v3</div>
